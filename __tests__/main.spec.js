...
beforeEach(() => {
	...
	jest.spyOn(dns, "resolveCname").mockResolvedValue([
		"host1.com",
		"host2.com",
	]);
...
describe("resolveUrlAndHost", () => {
	...
	the url doesn't exist`, async () => {
		...
		expect.assertions(1);
		await expect(
			mainModule.__get__("resolveUrlAndHost")(url)
		).rejects.toEqual(new Error("Invalid Host"));
	});
});
...
describe("resolveValidHost", () => {
	test("resolveValidHost resolve the canonical host", async () => {
		...
		const host = await rewire("../main").__get__("resolveValidHost")(
			urlObject
		);
...
test("defaultAwsCredentials returns the first environment variable with value in the list", async () => {
	...
	await mainModule.__with__({
		...
		expect(mainModule.__get__("defaultAwsCredentials")("awsKeys")).toBe(
			"key3"
		);
	});
	await mainModule.__with__({
